...
class Encoder(nn.Module):
    def __init__(self, args):
        ...
        # چند لایه‌ی کانولوشنی + BatchNorm + LeakyReLU
...
##############################################################################
"""تابع Borderline-SMOTE نمونه (برای جایگزینی SMOTE کلاسیک)"""
...
def borderline_smote(
    ...
    y: (n_samples,)
    minority_class: برچسب کلاس اقلیت
    n_to_sample: چند نمونه مصنوعی می‌خواهیم
    n_neighbors: تعداد همسایه برای تعیین مرزی بودن
    borderline_threshold: درصد همسایه‌هایی که اگر اکثریت بودند،
                         آن نمونه مرزی محسوب می‌شود
    """
...
for i in range(len(ids)):
    ...
    # چند تصویر تصادفی از دیتاست می‌گیریم و بازسازی آن‌ها را نشان می‌دهیم
    encoder.eval()
    decoder.eval()
    with torch.no_grad():
    ...
    for idx in range(8):
        # تصویر اصلی
        axes[0, idx].imshow(sample_images[idx].cpu().squeeze(), cmap='gray')
        axes[0, idx].set_title("Original")
        axes[0, idx].axis('off')
        # تصویر بازسازی‌شده
        axes[1, idx].imshow(recon_[idx].squeeze(), cmap='gray')
    ...
    #     مثلاً یک MLP کوچک در PyTorch یا LogisticRegression در sklearn
    # در اینجا یک طبقه‌بند خیلی ساده PyTorch برای داده‌ی MNIST
